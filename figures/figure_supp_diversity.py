...
### SCRIPT FOR PLOTTING SUPP FIGURE (DIVERSITY) IN FARRANKS PROJECT ###
...
if __name__ == "__main__":
	...
	## CONF ##
	prop_name = 'diversity' #rank property to consider
	...
	'dpi' : 300,
	'savename' : 'figure_supp_diversity' }
...
# A: Rank dependence of rank diversity in open/closed systems
	...
	for grid_pos, dataname in enumerate( fluxmean_data.index ): #loop through datasets (in order by decreasing mean flux)
		...
		if grid_pos in [ 0, 6, 12, 18, 24 ]:
			plt.ylabel( r'$d_R / d_m$', size=plot_props['xylabel'], labelpad=2 )
		...
		line, = plt.plot( xplot, yplot_data, c=datacols[ datatypes[ dataname ] ], lw=plot_props['linewidth'], rasterized=False, zorder=0 )

		#unfiltered model
		yplot_model = diversity_model / diversity_model.max() #normalise by max diversity!
		if dataname not in ignored_dsets: #if we have good sims...
		...
		#prepare legend
		if datatypes[ dataname ] not in leg_labels:
			leg_lines.append( line )
			leg_labels.append( datatypes[ dataname ] )
