...
### SCRIPT FOR PLOTTING SUPP FIGURE (SUCCESS) IN FARRANKS PROJECT ###
...
if __name__ == "__main__":
	...
	## CONF ##
	prop_name = 'success' #rank property to consider
...
# A: Time dependence of success in open/closed systems
	...
	for grid_pos, dataname in enumerate( fluxmean_data.index ): #loop through datasets (in order by decreasing mean flux)
		...
		if grid_pos in [ 0, 6, 12, 18, 24 ]:
			plt.ylabel( r'$S^{++}_t$', size=plot_props['xylabel'], labelpad=2 )
		...
		#get success time series in data
		...
		success_model = model_misc.success_theo( thres, params ) #success (theo)


		#plot plot!

		xplot = success_data.columns / float( T ) #normalised time = ( 0, ..., T-1 ) / T

		yplot_data = success_data.loc[ thres ] #success for given threshold
		line, = plt.plot( xplot, yplot_data, c=datacols[ datacats[ dataname ] ], lw=plot_props['linewidth'], rasterized=False, zorder=0 )

		#unfiltered model
		yplot_model = success_model #success for given threshold
...
#DEBUGGIN'
	...
	# ignored_dsets = [ 'TheGuardian_avgRecommends', 'github-watch-weekly', 'TheGuardian_numberComments', 'french', 'russian', 'german', 'spanish', 'italian', 'english', 'Tennis_ATP', 'Golf_OWGR', 'Poker_GPI', 'FIDEFemale', 'FIDEMale' ] #ignored datasets (while we run better sims!)
...
#	saveloc_model = root_loc+'nullModel/v4/files/model/'
		...
		#
		# success_model, not_used = model_misc.model_props( ['succ'], params, loadflag, saveloc_model )['succ']
