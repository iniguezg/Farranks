...
### SCRIPT FOR PLOTTING SUPP FIGURE (FLUX IN TIME) IN FARRANKS PROJECT ###
...
if __name__ == "__main__":
	...
	## CONF ##
	prop_name = 'flux_time' #rank property to consider
...
#	datasets = { 'VideogameEarnings' : 'economics', 'Virus' : 'nature' } #shady data
	...
	'dpi' : 300,
	'savename' : 'figure_supp_flux_time' }
...
# A: Time dependence of rank flux in open/closed systems
	...
	for grid_pos, dataname in enumerate( fluxmean_data.index ): #loop through datasets (in order by decreasing mean flux)
		...
		if grid_pos in [ 0, 6, 12, 18, 24 ]:
			plt.ylabel( r'$F_t$', size=plot_props['xylabel'], labelpad=2 )
...
#		#average flow
#		plt.axhline( y=yplot_data.mean(), label=None, c='0.7', ls='--', lw=plot_props['linewidth']-1, zorder=0 )
