...
### SCRIPT FOR PLOTTING SUPP FIGURE (MODEL RANK FLUX) IN FARRANKS PROJECT ###
...
if __name__ == "__main__":
	...
	saveloc_model = root_loc+'nullModel/v4/files/model/flux/' #location of output files
	...
	'dpi' : 300,
	'savename' : 'figure_supp_model_flux' }
...
# A: Ranking size dependence of rank flux vs. replacement probability in sims/theo
	...
	plt.ylabel( r'$F$', size=plot_props['xylabel'], labelpad=2 )
	...
	for pos_p0, p0 in enumerate( p0_vals ): #loop through p0 values
		...
		for pos_ptau, ptau in enumerate( ptau_vals ): #loop through ptau values
			...
			#first average OUT-/IN-flow over time
			#and then average over rank
			yplot_model[ pos_ptau ] = fluOprops_model.mean( axis=0 ).mean()

			yplot_theo[ pos_ptau ] = model_misc.flux_theo( params )
	...
	#legend
	leg = plt.legend( loc='lower right', bbox_to_anchor=(1, 0), title='$p =$', prop=plot_props['legend_prop'], handlelength=plot_props['legend_hlen'], numpoints=plot_props['legend_np'], columnspacing=plot_props[ 'legend_colsp' ], ncol=1 )
...
# B: Ranking size dependence of rank flux vs. diffusion probability in sims/theo
	...
	for pos_p0, p0 in enumerate( p0_vals ): #loop through p0 values
		...
		for pos_pnu, pnu in enumerate( pnu_vals ): #loop through pnu values
			...
			#first average OUT-/IN-flow over time
			#and then average over rank
			yplot_model[ pos_pnu ] = fluOprops_model.mean( axis=0 ).mean()

			yplot_theo[ pos_pnu ] = model_misc.flux_theo( params )
