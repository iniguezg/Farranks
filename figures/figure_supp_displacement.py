...
### SCRIPT FOR PLOTTING SUPP FIGURE (DISPLACEMENT) IN FARRANKS PROJECT ###
...
#	datasets = { 'VideogameEarnings' : 'open', 'Virus' : 'open' } #shady data
	...
	filter_factor = { 'AcademicRanking' : 15., 'AtlasComplex' : 10., 'Citations' : 10., 'Cities_RU' : 20., 'Cities_UK' : 30., 'Earthquakes_avgMagnitude' : 10., 'Earthquakes_numberQuakes' : 10., 'english' : 10., 'enron-sent-mails-weekly' : 20., 'FIDEFemale' : 200., 'FIDEMale' : 200., 'Football_FIFA' : 20., 'Football_Scorers' : 20., 'Fortune' : 40., 'french' : 10., 'german' : 10., 'github-watch-weekly' : 10., 'Golf_OWGR' : 200., 'Hienas' : 10., 'italian' : 10., 'metroMex' : 20., 'Nascar_BuschGrandNational' : 10., 'Nascar_WinstonCupGrandNational' : 20., 'Poker_GPI' : 40., 'russian' : 10., 'spanish' : 10., 'Tennis_ATP' : 200., 'TheGuardian_avgRecommends' : 10., 'TheGuardian_numberComments' : 10., 'UndergroundByWeek' : 40. } #factor for plotting filter
	...
	'fig_size' : (10, 6),
	'aspect_ratio' : (5, 6),
	'grid_params' : dict( left=0.1, bottom=0.09, right=0.99, top=0.88, wspace=0.3, hspace=0.7 ),
	'dpi' : 300,
	'savename' : 'figure_supp_displacement' }
...
# A: Displacement probability in open/closed systems
	...
	for grid_pos, dataname in enumerate( fluxmean_data.index ): #loop through datasets (in order by decreasing mean flux)
		...
		if grid_pos in [ 24, 25, 26, 27, 28, 29 ]:
			plt.xlabel( '$X / N_0$', size=plot_props['xylabel'], labelpad=2 )
		...
		dispprops_data = pd.read_pickle( saveloc_data + 'dispprops_' + param_str )
		...
		for pos_rank, rank in enumerate( dispprops_data.columns[ sel_pos ] ): #loop through selected ranks
			...
			if pos_rank == 0:
				label = '$R / N_0 = $ ' + label #fix 1st label

			#unfiltered/filtered data

			yplot_data = dispprops_data[ rank ] #get displacement for r
			filter_win = int( np.ceil( (N0 / filter_factor[dataname] ) / 2 ) * 2 ) + 1 #filter window len (odd int)
			yplot_data_filter = ss.savgol_filter( yplot_data, filter_win, 2 )

			plt.plot( xplot, yplot_data_filter, label=label, c=colors[ pos_rank ], lw=plot_props['linewidth'], rasterized=False, zorder=0 )

			#unfiltered model
			r = dr * ( rank + 1 ) #get r value
			yplot_model = model_misc.displacement_theo( r, params )
			plt.plot( xplot, yplot_model, '--', c='0.6', lw=plot_props['linewidth']-1, rasterized=False, zorder=1 )
	...
	#texts and arrows
	plt.text( -7.24, 7.25, 'open', va='center', ha='center', transform=ax.transAxes, weight='bold', rotation='vertical', fontsize=plot_props['xylabel'] )
	plt.text( -7.24, 0.5, 'closed', va='center', ha='center', transform=ax.transAxes, weight='bold', rotation='vertical', fontsize=plot_props['xylabel'] )
	plt.annotate( text='', xy=( -7.24, 1.1 ), xytext=( -7.24, 6.7 ), arrowprops=dict(arrowstyle='<->', color='0.6'), xycoords=ax.transAxes, textcoords=ax.transAxes )

	#legend
	plt.legend( loc='upper left', bbox_to_anchor=(-4.4, 9), prop=plot_props['legend_prop'], handlelength=plot_props['legend_hlen'], numpoints=plot_props['legend_np'], columnspacing=plot_props['legend_colsp'], ncol=len( sel_pos ) )
