...
### SCRIPT FOR PLOTTING SUPP FIGURE (OPENNESS) IN FARRANKS PROJECT ###
...
if __name__ == "__main__":
	...
	## CONF ##
	prop_name = 'openness' #rank property to consider
...
# A: Time dependence of openness in open/closed systems
	...
	for grid_pos, dataname in enumerate( fluxmean_data.index ): #loop through datasets (in order by decreasing mean flux)
		...
		#get openness time series in data
		...
		openness_model = model_misc.openness_theo( params ) #openness (theo)


		#plot plot!

		xplot = openness_data.index / float( T ) #normalised time = ( 0, ..., T-1 ) / T

		#unfiltered data
		yplot_data = openness_data
		line, = plt.semilogy( xplot, yplot_data, c=datacols[ datacats[ dataname ] ], lw=plot_props['linewidth'], rasterized=False, zorder=0 )

		#unfiltered model
		yplot_model = openness_model
		plt.semilogy( xplot, yplot_model, '--', c='0.5', lw=plot_props['linewidth']-2, rasterized=False, zorder=1 )
		...
		if grid_pos not in [ 0, 6, 12, 18, 24 ]:
			plt.yticks([])
		ax.locator_params( axis='x', nbins=4 ) #change number of ticks in axes
