...
### SCRIPT FOR PLOTTING SUPP FIGURE (FLUX IN RANK) IN FARRANKS PROJECT ###
...
if __name__ == "__main__":
	...
	## CONF ##
	prop_name = 'flux_rank' #rank property to consider
...
#	datasets = { 'VideogameEarnings' : 'economics', 'Virus' : 'nature' } #shady data
	...
	'dpi' : 300,
	'savename' : 'figure_supp_flux_rank_out' }
#	'savename' : 'figure_supp_flux_rank_in' }
...
# A: Time dependence of rank flux in open/closed systems
	...
	for grid_pos, dataname in enumerate( fluxmean_data.index ): #loop through datasets (in order by decreasing mean flux)
		...
		if grid_pos in [ 0, 6, 12, 18, 24 ]:
			plt.ylabel( r'$F^-_R$', size=plot_props['xylabel'], labelpad=2 )
#			plt.ylabel( r'$F^+_R$', size=plot_props['xylabel'], labelpad=2 )
...
#		fluIprops_data = pd.read_pickle( saveloc_data + 'fluIprops_' + param_str_data )
		fluO_data = fluOprops_data.mean( axis=0 ) #OUT-flow (averaged over time t)
#		fluI_data = fluIprops_data.mean( axis=0 ) #IN-flow (averaged over time t)
		...
		fluO_model = model_misc.flux_out_theo( params ) #OUT-flow (theo)
		...
		#OUT-flow
		yplot_data = fluO_data
		yplot_model = fluO_model
		#IN-flow
#		yplot_data = fluI_data
#		yplot_model = fluI_model
		...
		plt.text( 0.01, 0.99, fluxmean_str, va='top', ha='left', transform=ax.transAxes, fontsize=plot_props['text_size'])
	...
	plt.annotate( s='', xy=( -7.15, 1.1 ), xytext=( -7.15, 6.7 ), arrowprops=dict(arrowstyle='<->', color='0.6'), xycoords=ax.transAxes, textcoords=ax.transAxes )
...
#DEBUGGIN'
	...
	# ignored_dsets = [ 'TheGuardian_avgRecommends', 'github-watch-weekly' ] #ignored datasets (while we run better sims!)
		...
		#
		# print( 'ptau = {:.2f}, pnu = {:.2f}'.format( params['ptau'], params['pnu'] ) ) #to know where we stand
		#
		# fluOprops_model, fluIprops_model = model_misc.model_props( ['flux'], params, loadflag, saveloc_model )['flux']
		# fluO_model = fluOprops_model.mean( axis=0 ) #OUT-flow (averaged over time t)
		# fluI_model = fluIprops_model.mean( axis=0 ) #IN-flow (averaged over time t)
