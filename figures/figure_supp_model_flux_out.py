...
### SCRIPT FOR PLOTTING SUPP FIGURE (MODEL RANK OUT-FLUX) IN FARRANKS PROJECT ###
...
if __name__ == "__main__":
	...
	saveloc_model = root_loc+'nullModel/v4/files/model/flux_out/' #location of output files
	...
	'dpi' : 300,
	'savename' : 'figure_supp_model_flux_out' }
...
# A: ptau dependence of rank out-flux vs. rank in sims/theo
	...
	plt.xlabel( r'$r/p$', size=plot_props['xylabel'], labelpad=2 )
	plt.ylabel( r'$F^-_{r, t}$', size=plot_props['xylabel'], labelpad=2 )

	#plot plot!

	xplot = np.linspace( 1. / N0, 1, num=N0 ) #values of rank R / N_0
	...
	for pos_ptau, ptau in enumerate( ptau_vals ): #loop through ptau values
		...
		fluOprops_model, fluIprops_model = model_misc.model_props( ['flux'], params, loadflag, saveloc_model, saveflag )['flux']
		yplot_model = fluOprops_model.mean( axis=0 ) #OUT-flow (averaged over time t)

		yplot_theo = model_misc.flux_out_theo( params )

		#plot plot!

		plt.plot( xplot, yplot_model, 'o', label=label, c=colors[ pos_ptau ], ms=plot_props['marker_size'], rasterized=False )

		plt.plot( xplot, yplot_theo, label=None, c=colors[ pos_ptau ], lw=plot_props['linewidth'], rasterized=False )
...
# B: pnu dependence of rank out-flux vs. rank in sims/theo
	...
	for pos_pnu, pnu in enumerate( pnu_vals ): #loop through pnu values
		...
		plt.plot( xplot, yplot_model, 'o', label=label, c=colors[ pos_pnu ], ms=plot_props['marker_size'], rasterized=False )

		plt.plot( xplot, yplot_theo, label=None, c=colors[ pos_pnu ], lw=plot_props['linewidth'], rasterized=False )
